...
let handler = async (m, { conn, text }) => {
 if (!text) throw `🛡️ Ingresa el enlace de algún Track, PlayList o Álbum de Spotify.`; 
 let isSpotifyUrl = text.match(/^(https:\/\/open\.spotify\.com\/(album|track|playlist)\/[a-zA-Z0-9]+)/i);
 if (!isSpotifyUrl && !text) throw `🛡️ Ingresa el enlace de algún Track, Playlist o Álbum de Spotify.`
...
let txt = `*᥀🎧୭ S P O T I F Y - D O W N L O A D ᥀🎧୭*\n\n`
    ...
    txt += `*- ↻ Los audios se estan enviando espera un momento. . .*`
