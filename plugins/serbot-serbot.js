...
return m.reply("🛡️Este comando solo puede ser usado en el bot principal! wa.me/" + global.conn.user.jid.split`@`[0x0] + "?text=" + usedPrefix + "serbot")
}
...
async function serbot() {
    ...
    const connectionOptions = {
      ...
      browser: ['SOFIA-BOT', 'Safari', '2.0.0'],
      auth: {
        creds: state.creds,
    ...
    async function connectionUpdate(update) {
      ...
      if (qr) {
        let txt = '`᥀🤖୭  *S E R B O T - S U B B O T* ᥀🤖୭`\n\n'
            ...
            txt += `> *⚠️Nota⚠️:* Este código QR expira en 30 segundos.`
      ...
      if (connection == "open") {
        ...
        await parentw.reply(m.chat, args[0] ? '🌐Conectado con exito' : 'Conectado exitosamente con WhatsApp\n\n*Nota:* Esto es temporal\nSi el Bot principal se reinicia o se desactiva, todos los sub bots tambien lo haran\n\nEl número del bot puede cambiar, guarda este enlace:\n*-* https://whatsapp.com/channel/0029Vaec0hn3AzNS09nPJA2V', m, rcanal)
        await sleep(5000)
        if (args[0]) {
          return
        }
        await parentw.reply(conn.user.jid, "⚠️La siguiente vez que se conecte envía el siguiente mensaje para iniciar sesión sin escanear otro código *QR*", m, rcanal)
